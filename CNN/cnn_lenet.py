...
def load_mnist(fullset=False):
  ...
  if not fullset:
    m_train_small = m_train/200
    m_validate_small = m_validate/200
  ...
  for i in range(2, len(layers)+1):
    ...
    if layers[i]['type'] == 'CONV':
      ...
      weight = np.random.rand(
        layers[i]['k'] * layers[i]['k'] * c / layers[i]['group'],   #weight for conv,row number
        layers[i]['num']                                          # col number one col for one filter
      )
...
def pooling_layer_forward(input, layer):
  ...
  output = {}
  output['height'] = h_out  #12
  output['width'] = w_out    #12
  output['channel'] = c     #20
  output['batch_size'] = batch_size
  ...
  for n in range(batch_size):
    ...
    im = np.reshape(input_n['data'],(h_in,w_in,c))
    im = np.pad(im, ((pad, pad), (pad, pad), (0, 0)), mode='constant', constant_values=(0, 0))
    #col = np.zeros((k*k*c, h_out*w_out)).reshape()

    i = 0
    for h in range(h_out):
      for w in range(w_out):
        for q in range(c):
          matrix_hw = im[h*stride: h*stride+k, w*stride: w*stride+k, q]
          output['data'][i,n] = np.max(matrix_hw)
          i  += 1

    #temp = col.T.flatten.reshape(h_out*w_out,k*stride,c)
    #add = np.array([]);
    #for m in range(0,h_out*w_out):
    #  temp[m,]
  # implementation ends
...
def relu_forward(input, layer):
  ...
  depend = np.where(input['data']>0)
  output['data'][depend] = input['data'][depend]
...
def inner_product_forward(input, layer, param):
  ...
  # implementation begins
  output['data'] = param['w'].T.dot(input['data'])   #500x64
  for n in range(batch_size):
    output['data'][:, n] += param['b']
  # implementation ends
